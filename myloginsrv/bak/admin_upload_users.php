<?php
// Datei: admin_upload_users.php – Stand: 2025-04-24 12:15 Uhr Europe/Berlin
            ...
            foreach ($data as $user) {
                ...
                if ($username && $email && $password) {
                    ...
                    if ($existsU->fetchColumn() > 0) {
                        $importFailed++;
                        file_put_contents("error.log", date("c") . " Benutzername '$username' bereits vorhanden – übersprungen\n", FILE_APPEND);
                        continue;
                    }
                    if ($existsE->fetchColumn() > 0) {
                        $importFailed++;
                        file_put_contents("error.log", date("c") . " E-Mail '$email' bereits vorhanden – übersprungen\n", FILE_APPEND);
                        continue;
                    }
                    ...
                    $importInvalid++;
                    file_put_contents("error.log", date("c") . " Ungültiger Eintrag im JSON (fehlende Felder): " . json_encode($user) . "\n", FILE_APPEND);
                }
            }
        } else {
            echo "<div class='alert alert-danger small mt-2'>Fehler: Datei konnte nicht als gültiges JSON interpretiert werden.</div>";
        }
    }

    echo "<div class='alert alert-info small mt-2'>Import abgeschlossen: $importSuccess erfolgreich, $importFailed doppelt oder fehlgeschlagen, $importInvalid ungültig.</div>";
}
?>
...
<div class="row g-2 align-items-center mb-3">
    ...
    <div class="col-auto">
        <form method="post" enctype="multipart/form-data" class="d-flex align-items-center">
            <label class="form-label mb-0 me-2">Benutzerliste (.json):</label>
